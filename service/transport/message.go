...
// GetInterfaceArray of Message
func (m Message) GetInterfaceArray(key string) (val []interface{}, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]interface{})
	if !success {
		errorMessage := fmt.Sprintf("Conversion to interface{} array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
...
// GetFloat64Array of Message
func (m Message) GetFloat64Array(key string) (val []float64, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]float64)
	if !success {
		errorMessage := fmt.Sprintf("Conversion to float64 array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
...
// GetFloat32Array of Message
func (m Message) GetFloat32Array(key string) (val []float32, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]float32)
	if !success {
		errorMessage := fmt.Sprintf("Conversion to float32 array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
...
// GetInt64Array of Message
func (m Message) GetInt64Array(key string) (val []int64, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]int64)
	if !success {
		errorMessage := fmt.Sprintf("Conversion to int64 array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
...
// GetInt32Array of Message
func (m Message) GetInt32Array(key string) (val []int32, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]int32)
	if !success {
		errorMessage := fmt.Sprintf("Conversion to int32 array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
...
// GetIntArray of Message
func (m Message) GetIntArray(key string) (val []int, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]int)
	if !success {
		errorMessage := fmt.Sprintf("Conversion to int array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
...
// GetBoolArray of Message
func (m Message) GetBoolArray(key string) (val []bool, err error) {
	data, err := m.GetInterface(key)
	if err != nil {
		return val, err
	}
	val, success := data.([]bool)
	if !success {
		errorMessage := fmt.Sprintf("Conversion to bool array failed: %#v", val)
		err = errors.New(errorMessage)
	}
	return val, err
}
