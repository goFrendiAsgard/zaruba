...
```
  TASK NAME     : makeMongodbAppRunner
  LOCATION      : /zaruba-tasks/make/mongodb/task.makeMongodbAppRunner.yaml
  TASK TYPE     : Command Task
  PARENT TASKS  : [ makeDockerAppRunner ]
  DEPENDENCIES  : [ makeMongodbApp ]
  START         : - {{ .GetConfig "cmd" }}
                  - {{ .GetConfig "cmdArg" }}
  ...
  CONFIG        : _finish                      : Blank
                  ...
                  defaultAppDirectory          : {{ .ProjectName }}Mongodb
                  defaultAppPorts              : [
                                                   "27017"
                                                 ]
