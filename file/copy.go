// Copied and modified from: github.com/otiai10/copy v1.0.2
...
func CopyExcept(src, dest string, exceptions []string) error {
	...
	for _, exception := range exceptions {
		compiledException, err := regexp.Compile(exception)
		if err != nil {
			return err
		}
		compiledExceptions = append(compiledExceptions, compiledException)
	}
	return copy(src, dest, info, compiledExceptions)
}
...
func copy(src, dest string, info os.FileInfo, compiledExceptions []*regexp.Regexp) error {
	...
	if info.IsDir() {
		return dcopy(src, dest, info, compiledExceptions)
	}
...
func dcopy(srcdir, destdir string, info os.FileInfo, compiledExceptions []*regexp.Regexp) error {
	...
	for _, content := range contents {
		...
		if err := copy(cs, cd, content, compiledExceptions); err != nil {
