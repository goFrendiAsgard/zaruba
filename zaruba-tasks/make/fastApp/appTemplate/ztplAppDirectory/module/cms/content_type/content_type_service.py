...
class ContentTypeService():
    ...
    def find(
        ...
        rows = [
            self._fulfill(row)
            for row in self.content_type_repo.find(keyword, limit, offset)
        ]
    ...
    def update(
        ...
        updated_content_type = self.content_type_repo.update(
            id, content_type_data
        )
        self.mb.publish_activity(ActivityData(
            user_id=current_user.id,
            activity='update',
    ...
    def _validate_data(
        ...
        if content_type_data.name is not None:
            content_type = self.content_type_repo.find_by_name(
                content_type_data.name
            )
            if (
                content_type is not None
                and (id is None or content_type.id != id)
            ):
                raise HTTPException(
                    status_code=422,
                    detail=' '.join([
                        'Content type with the same name',
                        'already exist: {}'.format(content_type_data.name)
                    ])
                )
