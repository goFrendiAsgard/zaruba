...
from core.security.service.test_auth_service_util import (
    init_test_auth_service_components
)
...
def insert_activity_data(
    activity_repo: ActivityRepo, index: Optional[int] = None
) -> Activity:
    activity_data = create_activity_data()
    if index is None:
        activity_data.activity = 'activity'
    else:
        activity_data.activity = 'activity-{index}'.format(index=index)
    activity_data.created_by = 'original_user'
    activity_data.updated_by = 'original_user'
    return activity_repo.insert(activity_data)


def init_test_activity_service_components(
) -> Tuple[ActivityService, DBActivityRepo, AppMessageBus, AppRPC]:
