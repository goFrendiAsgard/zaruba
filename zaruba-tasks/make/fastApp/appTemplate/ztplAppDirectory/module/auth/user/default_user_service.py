...
class DefaultUserService(UserService):
    ...
    def find_by_identity_and_password(
        self, identity: str, password: str, current_user: Optional[User] = None
    ) -> Optional[User]:
        user = self.user_repo.find_by_identity_and_password(identity, password)
        if user is None:
            raise HTTPException(
                status_code=404,
                detail='identity or password does not match: {}'.format(
                    identity
                )
            )
        return user
    ...
    def insert(
        self, user_data: UserData, current_user: User
    ) -> Optional[User]:
    ...
    def update(
        self, id: str, user_data: UserData, current_user: User
    ) -> Optional[User]:
    ...
    def _validate_data(
        self, user_data: UserData, id: Optional[str] = None
    ) -> UserData:
        if user_data.username is not None:
            ...
            if user is not None and (id is None or user.id != id):
                raise HTTPException(
                    status_code=422,
                    detail='username already registered: {}'.format(
                        user_data.username
                    )
                )
        if user_data.email is not None:
            ...
            if user is not None and (id is None or user.id != id):
                raise HTTPException(
                    status_code=422,
                    detail='email already registered: {}'.format(
                        user_data.email
                    )
                )
        if user_data.phone_number is not None:
            user = self.user_repo.find_by_phone_number(
                user_data.phone_number
            )
            if user is not None and (id is None or user.id != id):
                raise HTTPException(
                    status_code=422,
                    detail='phone number already registered: {}'.format(
                        user_data.phone_number
                    )
                )
        return user_data
