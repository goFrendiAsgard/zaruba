...
from core.security.middleware.test_default_user_fetcher_util import init_test_default_user_fetcher_components
import pytest
...
async def test_default_user_fetcher_with_cookie():
    user_fetcher, _, _ = init_test_default_user_fetcher_components()
    fetch_user = user_fetcher.get_user_fetcher()
    user = await fetch_user(bearer_token=None, app_access_token='authorized_active')
    assert user == AUTHORIZED_ACTIVE_USER
