from module.auth.user.test_default_user_service_util import create_user
from core.token.test_jwt_token_service_util import init_test_jwt_token_service_components, ROOT_USER_DATA
...
def test_jwt_token_service_with_empty_token():
    ...
    user = token_service.get_user_by_cred_token('')
...
def test_jwt_token_service_with_null_token():
    token_service, _, _, _, _, _, _ = init_test_jwt_token_service_components()
    user = token_service.get_user_by_cred_token(None)
    # make sure token service return correct value
    assert user is None
...
def test_jwt_token_service_with_invalid_token():
    token_service, _, _, _, _, _, _ = init_test_jwt_token_service_components()
    user = token_service.get_user_by_cred_token('invalid token')
    # make sure token service return correct value
    assert user is None
...
def test_jwt_token_service_with_existing_user():
    ...
    # Creating token
    token = token_service.create_cred_token(root_user)
    # make sure token service return correct value
    assert token is not None
    assert token != ''
    # Get user
    user = token_service.get_user_by_cred_token(token)
    # make sure token service return correct value
...
def test_jwt_token_service_with_non_existing_user():
    ...
    # Creating token
    token = token_service.create_cred_token(inexist_user)
    # make sure token service return correct value
    assert token is not None
    assert token != ''
    # Get user
