...
def register_role_route(app: FastAPI, mb: MessageBus, rpc: RPC, auth_service: AuthService):
    ...
    @app.get('/roles/', response_service=List[Role])
    ...
    @app.get('/roles/{id}', response_service=Role)
    ...
    @app.post('/roles/', response_service=Role)
    ...
    @app.put('/roles/{id}', response_service=Role)
