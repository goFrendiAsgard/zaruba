...
def register_user_route(app: FastAPI, mb: MessageBus, rpc: RPC, auth_service: AuthService):
    ...
    @app.get('/users/', response_service=List[User])
    ...
    @app.get('/users/{id}', response_service=User)
    ...
    @app.post('/users/', response_service=User)
    ...
    @app.put('/users/{id}', response_service=User)
