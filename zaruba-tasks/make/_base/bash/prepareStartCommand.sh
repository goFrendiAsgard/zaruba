...
then
    ...
    then
        _ZRB_APP_START_ICON_COMMAND="go run main.go"
    elif [ -f "${_ZRB_APP_DIRECTORY}/package.json" ]
    then
        _ZRB_APP_START_ICON_COMMAND="npm start"
    elif [ -f "${_ZRB_APP_DIRECTORY}/main.py" ]
    then
        if [ -f "${_ZRB_APP_DIRECTORY}/Pipfile" ]
        then
            _ZRB_APP_START_ICON_COMMAND="pipenv run python main.py"
        elif [ -f "${_ZRB_APP_DIRECTORY}/venv/bin/activate" ]
        then
            _ZRB_APP_START_ICON_COMMAND="source ./venv/bin/activate && python main.py"
        else
            _ZRB_APP_START_ICON_COMMAND="python main.py"
        fi
    else
        _ZRB_APP_START_ICON_COMMAND="echo \"Replace this with command to start ${_ZRB_APP_NAME}\" && exit 1"
    fi
fi
